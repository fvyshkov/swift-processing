...
COMMENT ON TABLE public.process_type IS 'Process types for different SWIFT message types';
...
-- Process Types
INSERT INTO public.process_type (code, name_en, name_ru, name_combined, resource_url, attributes_table) VALUES
('pacs.008', 'Customer Credit Transfer', 'Клиентский кредитовый перевод', 'Customer Credit Transfer (Клиентский кредитовый перевод)', '/aoa/ObjectTask?object=swiftInput&form=editForm&objectKey={id}', 'swift_input'),
('pacs.009', 'Financial Institution Credit Transfer (COV)', 'Межбанковский кредитовый перевод (покрытие)', 'Financial Institution Credit Transfer (COV) (Межбанковский кредитовый перевод (покрытие))', '/aoa/ObjectTask?object=swiftInput&form=editForm&objectKey={id}', 'swift_input'),
('camt.053', 'Bank to Customer Statement', 'Банковская выписка клиенту', 'Bank to Customer Statement (Банковская выписка клиенту)', '/aoa/ObjectTask?object=swiftInput&form=editForm&objectKey={id}', 'swift_input'),
('camt.054', 'Bank to Customer Debit/Credit Notification', 'Уведомление о дебете/кредите', 'Bank to Customer Debit/Credit Notification (Уведомление о дебете/кредите)', '/aoa/ObjectTask?object=swiftInput&form=editForm&objectKey={id}', 'swift_input'),
('camt.056', 'FI to FI Payment Cancellation Request', 'Запрос на отмену платежа', 'FI to FI Payment Cancellation Request (Запрос на отмену платежа)', '/aoa/ObjectTask?object=swiftInput&form=editForm&objectKey={id}', 'swift_input'),
('TRN', 'Transaction', 'Транзакция (строка выписки)', 'Транзакция (строка выписки)', ' ', 'swift_stmt_ntry')
...
-- pacs.008 states
('f8c40da3-cf4e-42ec-a641-53eeb7208448', 'pacs.008', 'LOADED', 'Loaded', 'Загружен', 'Loaded (Загружен)', '#ccdec5', true, true, true),
('9f676606-51f2-4bbb-b220-88ae23f166c2', 'pacs.008', 'PROCESSED', 'Processed', 'Обработан', 'Processed (Обработан)', '#8bd672', false, false, false),
...
-- TRN states
('09835826-3239-4cde-8fdd-112f8e39c494', 'TRN', 'LOADED', 'Loaded', 'Загружена', 'Загружена', '#FF8C00', NULL, NULL, false),
('27b572c3-8bdf-42b7-bd43-999c3df7ba7d', 'TRN', 'PROCESSED', 'Processed', 'Обработана', 'Обработана', '#dbbbb8', NULL, NULL, false)
ON CONFLICT (id) DO NOTHING;

-- Process Operations (using UUIDs from backup)
INSERT INTO public.process_operation (id, type_code, code, name_en, name_ru, name_combined, icon, resource_url, availability_condition, cancel, to_state, move_to_state_script, workflow, database) VALUES
('e235c9a9-a22d-4ddb-ac59-3a54f9ad8d11', 'pacs.008', 'MARK_AS_PROCESSED', 'Mark as Processed', 'Отметить как обработанный', 'Mark as Processed (Отметить как обработанный)', 'check', NULL, '{"target_state": "PROCESSED", "available_in_states": ["LOADED"]}', false, NULL, 'to_state="PROCESSED"', NULL, NULL),
('04497cef-080c-41e4-8636-8b571bf9afb3', 'pacs.009', 'MARK_AS_PROCESSED', 'Mark as Processed', 'Отметить как обработанный', 'Mark as Processed (Отметить как обработанный)', 'check', NULL, '{"target_state": "PROCESSED", "available_in_states": ["LOADED"]}', false, NULL, 'to_state="PROCESSED"', NULL, NULL),
('3630a61c-2c21-44f1-9ea7-f7075327b14b', 'camt.053', 'MARK_AS_PROCESSED', 'Mark as Processed', 'Отметить как обработанный', 'Mark as Processed (Отметить как обработанный)', 'check', NULL, '{"target_state": "PROCESSED", "available_in_states": ["LOADED"]}', false, NULL, 'to_state="PROCESSED"', NULL, NULL),
('16d9608e-db6e-499e-ba57-2a3bffbf6481', 'camt.054', 'MARK_AS_PROCESSED', 'Mark as Processed', 'Отметить как обработанный', 'Mark as Processed (Отметить как обработанный)', 'check', NULL, '{"target_state": "PROCESSED", "available_in_states": ["LOADED"]}', false, NULL, 'to_state="PROCESSED"', NULL, NULL),
('ecaca1ee-dc5b-4c1e-87f9-79b488e09525', 'camt.056', 'MARK_AS_PROCESSED', 'Mark as Processed', 'Отметить как обработанный', 'Mark as Processed (Отметить как обработанный)', 'check', NULL, '{"target_state": "PROCESSED", "available_in_states": ["LOADED"]}', false, NULL, 'to_state="PROCESSED"', NULL, NULL),
('50e4a5c8-f510-4e45-bac3-f037c81545a6', 'pacs.008', 'CANCEL_PROCESSING', 'Cancel Processing', 'Отменить обработку', 'Cancel Processing (Отменить обработку)', 'undo', NULL, '{"target_state": "LOADED", "available_in_states": ["PROCESSED"]}', true, NULL, 'to_state="LOADED"', NULL, NULL),
('2405f16a-fcdb-4ef7-968e-7a41d5284e3e', 'pacs.009', 'CANCEL_PROCESSING', 'Cancel Processing', 'Отменить обработку', 'Cancel Processing (Отменить обработку)', 'undo', NULL, '{"target_state": "LOADED", "available_in_states": ["PROCESSED"]}', true, NULL, 'to_state="LOADED"', NULL, NULL),
('f159119f-7ea1-46e9-8339-ba6714f89174', 'camt.053', 'CANCEL_PROCESSING', 'Cancel Processing', 'Отменить обработку', 'Cancel Processing (Отменить обработку)', 'undo', NULL, '{"target_state": "LOADED", "available_in_states": ["PROCESSED"]}', true, NULL, 'to_state="LOADED"', NULL, NULL),
('eae080aa-61a6-4bd8-8056-1e53019188b5', 'camt.054', 'CANCEL_PROCESSING', 'Cancel Processing', 'Отменить обработку', 'Cancel Processing (Отменить обработку)', 'undo', NULL, '{"target_state": "LOADED", "available_in_states": ["PROCESSED"]}', true, NULL, 'to_state="LOADED"', NULL, NULL),
('ae4c638d-f954-4f3e-ac7a-c2ca7cd9ccb4', 'camt.056', 'CANCEL_PROCESSING', 'Cancel Processing', 'Отменить обработку', 'Cancel Processing (Отменить обработку)', 'undo', NULL, '{"target_state": "LOADED", "available_in_states": ["PROCESSED"]}', true, NULL, 'to_state="LOADED"', NULL, NULL),
('cd28fb8c-d732-4195-8c62-93001648552e', 'pacs.008', 'CANCEL_PAYMENT', 'Cancel Payment Creation', 'Отменить создание платежа', 'Cancel Payment Creation (Отменить создание платежа)', 'cancel', NULL, '{"target_state": "LOADED", "available_in_states": ["PAYMENT_CREATED"]}', true, NULL, 'to_state="LOADED"', NULL, NULL),
('2808dd8d-23c6-466d-b50a-d999268255ab', 'pacs.008', 'CREATE_PAYMENT', 'Create Payment', 'Создать платеж', 'Create Payment (Создать платеж)', 'payment', 'declare  p_dep_id int := 100;  p_id varchar2(250) := :id;  p_test_xml varchar2(4000):= :xml;begin  :out_payment_pk := p_dep_id||'',''||p_id;end;', '{"target_state": "PAYMENT_CREATED", "available_in_states": ["LOADED"]}', false, NULL, 'to_state="PAYMENT_CREATED"', 'type_008_payment', 'colvir_cbs'),
('4742683f-144d-4e7d-9596-0e0f9debf090', 'TRN', 'PROCESS', 'Process', 'Обработать транзакцию', 'Обработать транзакцию', NULL, '1', NULL, false, NULL, 'to_state = "PROCESSED"', '1', '1'),
('b07e6901-aacf-49bb-85c0-34c0fee379f3', 'TRN', 'UNDO_PROCESS', 'Undo Process', 'Отмена обработки транзакции', 'Отмена обработки транзакции', NULL, NULL, NULL, false, NULL, 'to_state = "LOADED"', NULL, NULL)
ON CONFLICT (id) DO NOTHING;
