...
<Envelope xmlns="urn:swift:xsd:envelope">
    ...
    pacs009_file_path = os.path.join(FOLDER_IN, 'pacs009_example.xml')
    try:
        xml_content = '''<?xml version="1.0" encoding="UTF-8"?>
<Envelope xmlns="urn:swift:xsd:envelope">
	<head:AppHdr xmlns:head="urn:iso:std:iso:20022:tech:xsd:head.001.001.02">
	...
	<pacs:Document xmlns:pacs="urn:iso:std:iso:20022:tech:xsd:pacs.009.001.08">
        ...
        logger.error(f'Error creating pacs.009 XML file: {e}')

    # Create camt.054 XML example file
    logger.debug('Creating camt.054 XML test file...')
    camt054_file_path = os.path.join(FOLDER_IN, 'camt054_example.xml')
    try:
        xml_content = '''<?xml version="1.0" encoding="UTF-8"?>
<Envelope xmlns="urn:swift:xsd:envelope">
	<head:AppHdr xmlns:head="urn:iso:std:iso:20022:tech:xsd:head.001.001.02">
	...
	<camt:Document xmlns:camt="urn:iso:std:iso:20022:tech:xsd:camt.054.001.08">
        ...
        logger.error(f'Error creating camt.054 XML file: {e}')

    # Create camt.056 XML example file
    logger.debug('Creating camt.056 XML test file...')
    camt056_file_path = os.path.join(FOLDER_IN, 'camt056_example.xml')
    try:
        xml_content = '''<?xml version="1.0" encoding="UTF-8"?>
<Envelope xmlns="urn:swift:xsd:envelope">
	<head:AppHdr xmlns:head="urn:iso:std:iso:20022:tech:xsd:head.001.001.02">
	...
	<camt:Document xmlns:camt="urn:iso:std:iso:20022:tech:xsd:camt.056.001.08">
        ...
        logger.error(f'Error creating camt.056 XML file: {e}')
        ...
        for filename in files:
            ...
            try:
                ...
                if msg_type == 'pacs.008':
                    ...
                    c.execute("""
                        INSERT INTO process (swift_input_id, state_id)
                        SELECT %s, id FROM process_state 
                        WHERE type_code = %s AND start = true
                        LIMIT 1
                ...
                # Delete file from folder_in after successful processing
                try:
                    os.remove(file_path)
